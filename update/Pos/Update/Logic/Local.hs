...
import           Pos.Update.Poll          (MonadPoll (deactivateProposal),
                                           ...
                                           filterProposalsByThd, getAdoptedBV,
                                           modifyPollModifier, normalizePoll, psVotes,
                                           refreshPoll, reportUnexpectedError, runDBPoll,
...
processSkeleton payload =
  ...
  where
    processSkeletonDo ms@MemState {..} = do
        modifier <-
            runDBPoll . evalPollT msModifier . execPollT def $ do
